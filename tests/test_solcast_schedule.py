...
def test_first_slot_is_5am() -> None:
    now = datetime(2026, 6, 21, 0, 0, tzinfo=_TZ)
    s = daily_schedule(now, _LAT, _LON)
    assert s[0].hour == 5 and s[0].minute == 0
...
def test_slots_strictly_inside_sunrise_sunset() -> None:
    ...
    s = daily_schedule(now, _LAT, _LON)
    daylight = [t for t in s if t != s[0]]  # drop the 5 AM slot
...
def test_slots_evenly_spaced_in_daylight() -> None:
    now = datetime(2026, 6, 21, 0, 0, tzinfo=_TZ)
    s = daily_schedule(now, _LAT, _LON)
    daylight = sorted(t for t in s if t.hour != 5 or t.minute != 0)
...
def test_falls_back_to_5am_only_without_coords() -> None:
    now = datetime(2026, 6, 21, 0, 0, tzinfo=_TZ)
